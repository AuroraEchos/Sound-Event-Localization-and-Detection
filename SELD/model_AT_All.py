...
class SELayer(nn.Module):
    """
    实现了一个简单的 Squeeze-and-Excitation（SE）模块，用于增强模型的表达能力。
    该模块包含一个全局平均池化层、两个全连接层和一个 Sigmoid 激活函数。
    通过这些层的组合，SE 模块可以学习到输入张量的通道间的相关性，从而提升模型的性能。
    """
...
class TC_Block(nn.Module):
    ...
    def __init__(self, in_channels, domain='DQ', 
        ...
        if attention_type=='self_attention':
            self.attention = ATLayer(in_channels=V[1], key_size=key_size, value_size=value_size)
        elif attention_type=='squeeze_excitation':
            self.attention = SELayer(in_channels=V[1])
        else:
            self.attention = None
        """
        从第145行到第169行：
...
class ConvTC_Block(nn.Module):
    ...
    def __init__(self, time_dim, freq_dim=256, input_channels=4, 
        ...
        if attention_type=='self_attention':
            self.attention = ATLayer(in_channels=L, key_size=key_size, value_size=value_size)
        elif attention_type=='squeeze_excitation':
            self.attention = SELayer(in_channels=L)
        else:
            self.attention = None
